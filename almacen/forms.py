...
class registro_form(forms.ModelForm):
    ...
    formato = forms.ChoiceField(label='Formato', choices=acervo_model.Format.choices, required=True, widget=forms.Select (attrs={'class':'form-control','placeholder':'Indique el formato del libro'}))
    estado = forms.ChoiceField(label='Estado', choices=acervo_model.State.choices, required=True, widget=forms.Select (attrs={'class':'form-control','placeholder':'Indique el estado del libro'}))
    class Meta:
        model = acervo_model
